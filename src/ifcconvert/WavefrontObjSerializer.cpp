...
WaveFrontOBJSerializer::WaveFrontOBJSerializer(
    const std::string &obj_filename, const std::string &mtl_filename,
    const SerializerSettings &settings)
    : GeometrySerializer(settings), mtl_filename(mtl_filename),
...
void WaveFrontOBJSerializer::writeMaterial(const IfcGeom::Material &style)
{
  std::string material_name =
      (settings().get(SerializerSettings::USE_MATERIAL_NAMES)
           ? style.original_name()
           : style.name());
  ...
  if (style.hasSpecular())
  {
    ...
    mtl_stream << "Ks " << specular[0] << " " << specular[1] << " "
               << specular[2] << "\n";
  }
    ...
    const IfcGeom::TriangulationElement<real_t> *o)
{
  ...
  for (std::vector<real_t>::const_iterator it = mesh.verts().begin();
       it != mesh.verts().end();)
  {
  ...
  for ( std::vector<real_t>::const_iterator it = mesh.normals().begin(); it
  != mesh.normals().end(); ) {
      const real_t x = *(it++);
      const real_t y = *(it++);
      const real_t z = *(it++);
     ...
     for (std::vector<real_t>::const_iterator it = mesh.uvs().begin(); it !=
     mesh.uvs().end();) {
      const real_t u = *it++;
      const real_t v = *it++;
      obj_stream << "vt " << u << " " << v << "\n";
       ...
       it != mesh.faces().end();)
  {
    ...
    {
      obj_stream << "f " << v1 << "/" << v1 << "/" << v1 << " " << v2 << "/"
                 << v2 << "/" << v2 << " " << v3 << "/" << v3 << "/" << v3
                 << "\n";
    }
    else if (has_normals)
    {
      obj_stream << "f " << v1 << "//" << v1 << " " << v2 << "//" << v2 << " "
                 << v3 << "//" << v3 << "\n";
    }
    else
    {
  ...
  for ( std::vector<int>::const_iterator it = edges.begin(); it !=
  edges.end(); ) { 	const int i1 = *(it++); 	const int i2 = *(it++);

  	if (faces_set.find(i1) != faces_set.end() || faces_set.find(i2) !=
  faces_set.end()) { 		continue;
  	}
