...
public:
  explicit Representation(const ElementSettings &settings) : settings_(settings)
  {
  }
...
public:
  BRep(const ElementSettings &settings, const std::string &id,
       const IfcGeom::IfcRepresentationShapeItems &shapes)
      : Representation(settings), id_(id), shapes_(shapes)
  {
  }
  virtual ~BRep() {}
...
template <typename P> class Triangulation : public Representation
{
private:
  // A nested pair of floats and a material index to be able to store an XYZ
  // coordinate in a map.
...
public:
  ...
  Triangulation(const BRep &shape_model)
      : Representation(shape_model.settings()), id_(shape_model.id())
  {
    for (IfcGeom::IfcRepresentationShapeItems::const_iterator iit =
             shape_model.begin();
         iit != shape_model.end(); ++iit)
    {
      ...
      if (iit->hasStyle())
      {
        ...
        std::vector<Material>::const_iterator jt =
            std::find(_materials.begin(), _materials.end(), adapter);
      ...
      if (settings().get(IteratorSettings::APPLY_DEFAULT_MATERIALS) &&
          surface_style_id == -1)
      {
        Material material(
            IfcGeom::get_default_style(settings().element_type()));
        std::vector<Material>::const_iterator mit =
            std::find(_materials.begin(), _materials.end(), material);
...
const Standard_Boolean isRelative = Standard_False;
      ...
      catch (...)
      {
        ...
        // TODO: Catch outside
        // Logger::Message(Logger::LOG_ERROR,"Failed to triangulate
        // shape:",ifc_file->entityById(_id)->entity);
      ...
      for (exp.Init(s, TopAbs_FACE); exp.More(); exp.Next(), ++num_faces)
      {
        ...
        if (!tri.IsNull())
        {
          ...
          const bool calculate_normals =
              !settings().get(IteratorSettings::WELD_VERTICES) &&
          ...
          for (int i = 1; i <= nodes.Length(); ++i)
          {
            ...
            if (calculate_normals)
            {
              ...
              else
              {
                ...
                // TODO: Do the same for conical surfaces, but they are rare in
                // IFC.
              }
          ...
          for (std::vector<std::pair<int, int>>::const_iterator jt =
                   edges_temp.begin();
               jt != edges_temp.end(); ++jt)
          {
            if (edgecount[*jt] == 1)
            {
      ...
      if (!_normals.empty() &&
          settings().get(IfcGeom::IteratorSettings::GENERATE_UVS))
      {
      ...
      if (num_faces == 0)
      {
        // Edges are only emitted if there are no faces. A mixed representation
        // of faces and loose edges is discouraged by the standard. An
        // alternative would be to use TopExp_Explorer texp(s, TopAbs_EDGE,
        // TopAbs_FACE) to find edges that do not belong to any face.
        for (TopExp_Explorer texp(s, TopAbs_EDGE); texp.More(); texp.Next())
        {
          BRepAdaptor_Curve crv(TopoDS::Edge(texp.Current()));
          GCPnts_QuasiUniformDeflection tessellater(
              crv, settings().deflection_tolerance());
  ...
  static std::vector<P> box_project_uvs(const std::vector<P> &vertices,
                                        const std::vector<P> &normals)
  {
    std::vector<P> uvs;
    uvs.resize(vertices.size() / 3 * 2);
    for (size_t uv_idx = 0, v_idx = 0;
         uv_idx < uvs.size() && v_idx < vertices.size() &&
         v_idx < normals.size();
         uv_idx += 2, v_idx += 3)
    {

      P n_x = normals[v_idx], n_y = normals[v_idx + 1],
        n_z = normals[v_idx + 2];
      P v_x = vertices[v_idx], v_y = vertices[v_idx + 1],
...
private:
  ...
  int addVertex(int material_index, const gp_XYZ &p)
  {
    ...
    const P X =
        static_cast<P>(convert ? (p.X() / settings().unit_magnitude()) : p.X());
    const P Y =
        static_cast<P>(convert ? (p.Y() / settings().unit_magnitude()) : p.Y());
    const P Z =
        static_cast<P>(convert ? (p.Z() / settings().unit_magnitude()) : p.Z());
    int i = (int)_verts.size() / 3;
    if (settings().get(IteratorSettings::WELD_VERTICES))
    {
      const VertexKey key = std::make_pair(
          material_index, std::make_pair(X, std::make_pair(Y, Z)));
                      ...
                      std::vector<std::pair<int, int>> &edges_temp)
  {
    ...
    if (edgecount.find(e) == edgecount.end())
      edgecount[e] = 1;
    else
      edgecount[e]++;
