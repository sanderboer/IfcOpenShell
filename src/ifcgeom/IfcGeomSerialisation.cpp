...
#ifdef USE_IFC4
IfcSchema::IfcKnotType::IfcKnotType
opencascade_knotspec_to_ifc(GeomAbs_BSplKnotDistribution bspline_knot_spec)
{
  IfcSchema::IfcKnotType::IfcKnotType knot_spec =
                   ...
                   IfcSchema::IfcSurface *&surface, bool advanced)
{
  ...
  else if (s->DynamicType() == STANDARD_TYPE(Geom_BSplineSurface))
  {
    ...
    IfcSchema::IfcKnotType::IfcKnotType knot_spec_u =
        opencascade_knotspec_to_ifc(bspline->UKnotDistribution());
    IfcSchema::IfcKnotType::IfcKnotType knot_spec_v =
...
IfcGeom::serialise(const TopoDS_Shape &shape, bool advanced)
{
#ifndef USE_IFC4
  advanced = false;
#endif
  ...
  for (TopExp_Explorer exp(shape, TopAbs_SOLID); exp.More(); exp.Next())
  {
    ...
    if (advanced)
    {
      ...
      else
      {
        ...
        /// @todo this is not necessarily correct as the shell is not
        /// necessarily facetted.
        if (inner->size())
    {
  ...
  if (items->size() > 0)
  {
    rep = new IfcSchema::IfcShapeRepresentation(0, std::string("Body"),
                                                std::string("Brep"), items);
  }
  else
  {

    // If not, see if there is a shell
    IfcSchema::IfcOpenShell::list::ptr shells(
        new IfcSchema::IfcOpenShell::list);
    ...
    {
      items->push(
          new IfcSchema::IfcShellBasedSurfaceModel(shells->generalize()));
      rep = new IfcSchema::IfcShapeRepresentation(0, std::string("Body"),
                                                  std::string("Brep"), items);
    }
    else
      ...
      else
      {
        ...
        else
        {
          ...
          IfcSchema::IfcGeometricCurveSet *curves =
              new IfcSchema::IfcGeometricCurveSet(edges);
          items->push(curves);
          rep = new IfcSchema::IfcShapeRepresentation(
...
IfcSchema::IfcProductDefinitionShape *
IfcGeom::tesselate(const TopoDS_Shape &shape, double deflection)
{
  ...
  BRepMesh_IncrementalMesh(shape, deflection, isRelative, theAngDeflection,
                           isInParallel);
