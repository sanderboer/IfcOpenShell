...
IfcGeom::Representation::Serialization::Serialization(const BRep &brep)
    : Representation(brep.settings()), id_(brep.id()) {
  TopoDS_Compound compound = brep.as_compound();
  for (IfcGeom::IfcRepresentationShapeItems::const_iterator it = brep.begin();
       it != brep.end(); ++it) {
...
TopoDS_Compound IfcGeom::Representation::BRep::as_compound() const {
  ...
  for (IfcGeom::IfcRepresentationShapeItems::const_iterator it = begin();
       it != end(); ++it) {
    ...
    const TopoDS_Shape moved_shape =
        IfcGeom::Kernel::apply_transformation(s, trsf);
